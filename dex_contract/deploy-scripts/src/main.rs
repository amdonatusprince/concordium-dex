...
async fn main() -> Result<(), Error> {
    ...
    use dex_contract::CustomInputParameter; // Example

    let param = OwnedParameter::from_serial(&CustomInputParameter { num: 42 })?; // Example

    let init_method_name: &str = "init_dex_contract"; // Example
    ...
    let update_payload = transactions::UpdateContractPayload {
        ...
        receive_name: OwnedReceiveName::new_unchecked("dex_contract.receive".to_string()),
